...
def sosiUpdate():
    ...
    if row:
        ...
        if a == '코드':
            ...
            msg = f"UPDATE sosi SET code = {name} where code = {dcode}"
            cursor.execute(msg)
            conn.commit()
        elif a == '제목':
            title = input('어떻게 수정하시겠습니까? >> ')
            msg = f"UPDATE sosi SET code = {title} where code = {dcode}"
            cursor.execute(msg)
            conn.commit()
        elif a == '급여':
            sal = int(input('어떻게 수정하시겠습니까? >> '))
            msg = f"UPDATE sosi SET code = {sal} where code = {dcode}"
            cursor.execute(msg)
            conn.commit()
        elif a == '등급':
            grade = input('어떻게 수정하시겠습니까? >> ').upper()
            msg = f"UPDATE sosi SET code = {grade} where code = {dcode}"
            cursor.execute(msg)
            conn.commit()
        else:
